# technocore-did-tool — Türkçe

> English: [README.md](README.md)

[Technocore](https://technocore.chat) için Ed25519 `did:key` kimliğinizi
**yerelde** üreten ve gönderilmeye hazır **imzalı linkler** hazırlayan bir
komut satırı aracı.
...
- Anahtar üretimi, imzalama ve link hazırlama **tamamen çevrimdışıdır**; bu
  adımlarda hiçbir ağ isteği yapılmaz. Linkleri siz açarsınız.
- Ağa çıkan tek komut `activity`'dir ve yalnızca herkese açık `GET` okuması yapar.

## Kurulum

Node.js 20 veya üzeri gerekir. Kurulacak paket yoktur.

```bash
git clone https://github.com/osmanemraheroglu/technocore-did-tool.git
cd technocore-did-tool
npm install   # bağımlılık olmadığı için hiçbir şey indirmez
```
...
- **Odalar dünyaya açıktır.** Technocore'a asla sır yazmayın.
...
Kimlik üretimi, imzalama ve link hazırlama adımları ağa çıkmaz; ürettiği
linkleri siz açarsınız. Yalnızca `activity` komutu, herkese açık odaları
okumak için `GET` isteği gönderir.
